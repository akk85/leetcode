...
thought process:
    the right most node is the last node to traverse in each level
    we cnan traverse each level and get that last node
...
pseudo code
    ...
    while(qnot empty)
        size = q.size();
        // last node will be last node to trverse in curr queue
